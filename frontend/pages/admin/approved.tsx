...
function ApprovedView() {
  ...
  return (
    <div className="rounded-2xl p-4 bg-white/95 backdrop-blur border border-gray-100 max-w-6xl mx-auto">
      <div className="mb-4">
        <PageHeader title="Admin • Approved Bookings" subtitle="Approved bookings overview" />
        <div className="mb-3">
          <AdminTabs />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState title="No approved bookings" subtitle="Approved bookings will appear here" />
      ) : (
        <div className="space-y-3">
          {rows.map((r) => (
            <div key={r._id} className="rounded-lg border border-green-200 bg-white/95 backdrop-blur shadow-md hover:shadow-lg transition">
              ...
              {expanded[r._id] && (
                <div className="p-4 border-t border-green-200">
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={() => { setConfirmTarget({ id: r._id, action: 'pending' }); setConfirmOpen(true); }} className="btn btn-secondary">Revoke ↩</button>
                  </div>
                </div>
              )}
            </div>
